...
scripts/set-lynx-version.sh 3.9.0           # Podfile 버전 일괄 갱신 (PrimJS는 major.minor 매칭)
...
버전은 `Podfile`에 고정되어 있다 (Lynx 3.6.0 / PrimJS 3.6.1 / LynxService 3.6.0 /
SDWebImage 5.15.5 / SDWebImageWebPCoder 0.11.0 / XElement 3.6.0, iOS 12.0).
XElement는 pod 의존성으로 빌드되지만 build.sh의 추출 대상(FRAMEWORKS)에는 포함되어 있지 않다.
...
- 릴리스 태그 = Lynx 버전 (예: `3.9.0`). PrimJS는 같은 major.minor 중 최신을 자동 선택한다.
